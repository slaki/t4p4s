...
def header_instance_byte_width():
    ws = [str(hi.header_type.length) if isinstance(hi.header_type.length, int) else "-1 /* variable length header */" for hi in header_instances(hlir)]
...
def header_instance_variable_field():
    return harray("int", variable_length_field_ids(hlir))

def field_instance_bit_width():
    ws = ["-1 /* variable length field */" if f[1] == p4.P4_AUTO_WIDTH else str(f[1]) for hi in header_instances(hlir) for f in hi.header_type.layout.items()]
...
#[ ${ header_instance_variable_field() }
