...
#define MODIFY_INT32_INT32_HTON(pd, dstfield, value32) { \
    if(field_desc(dstfield).bytecount == 1) \
        ...
        res32 = (FIELD_BYTES(pd, dstfield) & ~field_desc(dstfield).mask) | (rte_cpu_to_be_16(value32 << (16 - FIELD_BITCOUNT(dstfield))) & field_desc(dstfield).mask); \
    else \
        res32 = (FIELD_BYTES(pd, dstfield) & ~field_desc(dstfield).mask) | (rte_cpu_to_be_32(value32 << (32 - FIELD_BITCOUNT(dstfield))) & field_desc(dstfield).mask); \
    memcpy(FIELD_BYTE_ADDR(pd, field_desc(dstfield)), &res32, field_desc(dstfield).bytecount); \
}
...
#define GET_INT32_AUTO(pd, field) (field_desc(field).meta ? \
    ...
    (rte_be_to_cpu_32((FIELD_BYTES(pd, field) & field_desc(field).mask)) >> (32 - FIELD_BITCOUNT(field))))
...
#define EXTRACT_INT32_NTOH(pd, field, dst) { \
    if(field_desc(field).bytecount == 1) \
        ...
        dst = rte_be_to_cpu_16((FIELD_BYTES(pd, field) & field_desc(field).mask)) >> (16 - FIELD_BITCOUNT(field)); \
    else \
        dst = rte_be_to_cpu_32((FIELD_BYTES(pd, field) & field_desc(field).mask)) >> (32 - FIELD_BITCOUNT(field)); \
}
